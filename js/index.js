...
const hardGame1 = document.getElementById("hardGame1");
...
function startGame1() {
  ...
  console.log(gameMode, mediumGame);
  if (gameMode === "easy") {
    cards = deckCards1.slice(0, 4);
    deck.classList.add("easy");
  } else if (gameMode === "medium") {
    deck.classList.add("medium");
    cards = deckCards1.slice(0, 8);
  } else {
    deck.classList.add("hard");
    cards = [...deckCards1];
  }
...
function AddStats() {
  ...
  p[0].innerHTML = `かかった時間: ${minutes}分 ${seconds}秒 (最高記録: ${best_minute}分 ${best_second}秒)`;
  p[1].innerHTML = `移動回数: ${moves} (最高記録: ${best_moves})`;
}
...
hardGame1.addEventListener("click", function () {
  startModal.style.display = "none";
  gameMode = "hard";
  console.log("ENTROU");
  startGame1();
...
liTag.addEventListener("click", (event) => {
  liTag.style.transform = "scaleX(-1)";
});
